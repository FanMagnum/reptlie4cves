...
print(f"proxy ip pool {proxy_ip_pool}")
...
def get_one_page(index, vendor, product, version):
    try:
        ...
        # r = requests.get(url, params=params, headers=headers, allow_redirects=False, proxies=random.choice(proxy_ip_pool))
        r = requests.get(url, params=params, headers=headers, allow_redirects=False)
        r.raise_for_status()
        r.encoding = r.apparent_encoding
